...
std::string recognizeCharsInPlate(cv::Mat &imgThresh, std::vector<PossibleChar> &vectorOfMatchingChars) {
    ...
    for (auto &currentChar : vectorOfMatchingChars) {           // for each char in plate
        ...
        kNearest->findNearest(matROIFlattenedFloat, 1, matCurrentChar, matResponse,matDistance);     // finally we can call find_nearest !!!

        float fltCurrentChar = (float)matCurrentChar.at<float>(0, 0);       // convert current char from Mat to float

		float fltResponseChar = (float)matResponse.at<float>(0, 0);

		float fltDistance = (float)matDistance.at<float>(0, 0);       // convert current char from Mat to float

		std::cout << "\nthe distance is:" << fltDistance;

		strChars2 = strChars2 + char(int(fltResponseChar));

		std::cout << "\n the nearest response is: " << strChars2;
		if (int(fltDistance) < 10000000)
		{
